...
Copy **`.env.example`** → **`.env`** and set **`VITE_API_LOC`** (your API root URL, no trailing slash). Restart **`npm run dev`** after editing **`.env`**. Optional Supabase keys for Auth go in the same file.
...
The app reads your project URL and the public anon key from the environment so the browser can use Supabase Auth. Add the same keys to your **`.env`** (or **`.env.local`**) — not committed to git.

In the Supabase dashboard, open Project Settings → API and copy the project URL and anon/public key:
